...
void UpdateMachine(void)
{
	...
	{
		if (pMachine->bUse == true)	//使っていたら
		{
			switch (pMachine->State)
			{
			case MACHINESTATE_APPEAR:	//子機アイテム出現

				//位置の更新
				pMachine->pos.y += 5;

				if (pMachine->pos.y > SCREEN_HEIGHT)	//画面下に行き見えなくなったら
				{
					pMachine->State = MACHINESTATE_BREAK;		//アイテムを消去する
				}


				break;
			
			case MACHINESTATE_NORMAL:	//ノーマル状態
			...
			case MACHINESTATE_BREAK:	//壊す

				pMachine->bUse = false;	//使ってない状態に

				break;

			}

			SetVertexMachine(nCntMachine);	//子機の頂点座標
		}
