...
HRESULT InitTime(void)
{
	...
	TimerPos = D3DXVECTOR3(0.0f, 0.0f, 0.0f);	//タイマーの位置						
	if (pGame->stage == STAGE_1)
	{
		g_nTimer.nTime = TIME_STAGE1;
	}
	else
	{
		g_nTimer.nTime = TIME_STAGE2;
	}
	g_nTimer.bTimeup = false;
	g_nCounterTime = 0;
	...
	{
		//ポリゴンの位置
		TimerPos = D3DXVECTOR3(910.0f + nCntTime* 50, 50.0f, 0.0f);

		//頂点座標
		pVtx[0].pos = D3DXVECTOR3(TimerPos.x - TIME_SIZEX, TimerPos.y + TIME_SIZEY, 0.0f);
		pVtx[1].pos = D3DXVECTOR3(TimerPos.x - TIME_SIZEX, TimerPos.y - TIME_SIZEY, 0.0f);
		pVtx[2].pos = D3DXVECTOR3(TimerPos.x + TIME_SIZEX, TimerPos.y + TIME_SIZEY, 0.0f);
		pVtx[3].pos = D3DXVECTOR3(TimerPos.x + TIME_SIZEX, TimerPos.y - TIME_SIZEY, 0.0f);
		...
		//時間の計算
		g_aNumber[nCntTime] = (g_nTimer.nTime / powf(10.0f, MAX_TIME - (nCntTime + 1.0f)));
...
void UpdateTime(void)
{
	...
	{
		g_aNumber[nCntTime] = (g_nTimer.nTime / powf(10, MAX_TIME - (nCntTime + 1)));
